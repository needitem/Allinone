...
public class TPUtils {
    ...
    public static ArrayList<Vec3d> findTPPath(Vec3d startPos, Vec3d newPos, int maxSteps, double maxDistance,
            double accuracy) {
        ...
        for (int h = 0; h < maxSteps; h++) {
            double closestPosSquaredDistance = positions.isEmpty() ? Integer.MAX_VALUE
                    : (positions.getLast().squaredDistanceTo(newPos) + 100);
            Vec3d currentPos = positions.isEmpty() ? startPos : positions.getLast();
            double currentPosDistance = currentPos.distanceTo(newPos);
            Vec3d roundCurrentPos = new Vec3d(Math.floor(currentPos.x) + 0.5, Math.floor(currentPos.y),
            ...
            if (isTPValid(currentPos, potentialPos)) {
                ...
                positionsLoop: for (int i = 10; i >= -10; i--) {
                    int maxJ = (int) Math.sqrt(100 - i * i);
                    for (int j = maxJ; j >= -maxJ; j--) {
                        int maxK = (int) Math.sqrt(100 - i * i - j * j);
                        for (int k = maxK; k >= -maxK; k--) {
                            potentialPos = roundCurrentPos.offset(directions.get(0), i).offset(directions.get(1), j)
                                    .offset(directions.get(2), k);
                            double potentialPosSquaredDistance = potentialPos.squaredDistanceTo(newPos);
                            if (potentialPosSquaredDistance < closestPosSquaredDistance
                                    && isTPValid(currentPos, potentialPos)) {
                ...
                if (closestPos != null && !positions.contains(closestPos)
                        && !(closestPosSquaredDistance > 10 * Math.pow(maxSteps - h, 2 + maxDistance))) {
    ...
    public static boolean isTPValid(Vec3d startPos, Vec3d endPos) {
        return startPos.squaredDistanceTo(endPos) < 100.0000000000001 && !isObstructed(endPos)
