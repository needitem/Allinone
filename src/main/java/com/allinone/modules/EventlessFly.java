...
import com.allinone.mixin.PlayerPositionLookS2CPacketAccessor;
import com.allinone.utils.ServerUtils;
...
public class EventlessFly extends Module {
    ...
    public void onDeactivate() {
        if (cancelRotations.get() && resetRotationOnDisable.get()) {
            ...
            mc.player.networkHandler.sendPacket(
                    new PlayerMoveC2SPacket.LookAndOnGround(cameraYaw, cameraPitch, mc.player.isOnGround()));
        }
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        mc.player.networkHandler.sendPacket(
                new PlayerMoveC2SPacket.PositionAndOnGround(endPos.x, endPos.y, endPos.z, mc.player.isOnGround()));
        mc.player.networkHandler
                .sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(endPos.x, -1E6, endPos.z, false));
    }

    @EventHandler
    private void onPacketReceive(PacketEvent.Receive event) {
        if (mc.player == null || !cancelRotations.get())
            return;
