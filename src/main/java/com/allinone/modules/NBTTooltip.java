...
import com.allinone.utils.NBTUtils;
...
public class NBTTooltip extends Module {
    ...
    public String getTooltip(ItemStack stack, Item.TooltipContext ctx) {
        ...
        if (isActive() && (!onlyOnKey.get() || displayKey.get().isPressed())) {
            DataResult<NbtElement> result = ComponentChanges.CODEC
                    .encodeStart(ctx.getRegistryLookup().getOps(NbtOps.INSTANCE), stack.getComponentChanges());
            result.ifError(e -> {
            });
