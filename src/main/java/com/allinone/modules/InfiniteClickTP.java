...
public class InfiniteClickTP extends Module {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (mc.options.sneakKey.isPressed() || !onlyIfSneaking.get()) {
            ...
            RaycastContext context = new RaycastContext(mc.player.getEyePos(),
                    mc.player.getEyePos().add(mc.player.getRotationVector().multiply(maxDistance.get())),
            ...
            while (!Streams.stream(mc.world.getBlockCollisions(mc.player, box.offset(Vec3d.ofBottomCenter(blockPos))))
                    .toList().isEmpty()) {
            ...
            if (mode.get() == Mode.Paper) {
                wrongMove = mc.player.getPos().squaredDistanceTo(tpPos) > 40_000
                        || TPUtils.isWrongMove(mc.player.getPos(), tpPos);
            ...
            if (mode.get() != Mode.Pathfinder && preventWrongMoves.get() && wrongMove)
                return;
                ...
                switch (mode.get()) {
                    ...
                    case Straight -> {
                        ...
                        mc.player.networkHandler.sendPacket(
                                new PlayerMoveC2SPacket.PositionAndOnGround(tpPos.x, tpPos.y, tpPos.z, true));
                        positions.add(tpPos);
                    }
                    ...
                    case Pathfinder -> {
                        ...
                        if (steps != null) {
                            ...
                            for (Vec3d vec : steps) {
                                mc.player.networkHandler.sendPacket(
                                        new PlayerMoveC2SPacket.PositionAndOnGround(vec.x, vec.y, vec.z, true));
                            }
                            mc.player.networkHandler.sendPacket(
                                    new PlayerMoveC2SPacket.PositionAndOnGround(tpPos.x, tpPos.y, tpPos.z, true));
                        }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (lastTP + renderTime.get() * 1000 < System.currentTimeMillis())
            return;
        ...
        if (showSteps.get()) {
            for (Vec3d vec : positions) {
                event.renderer.box(vec.x - 0.375, vec.y, vec.z - 0.375, vec.x + 0.375, vec.y + 2, vec.z + 0.375,
                        stepColor.get(), stepColor.get(), ShapeMode.Sides, 0);
            }
        }
    }
