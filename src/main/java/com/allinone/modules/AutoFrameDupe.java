...
import com.allinone.utils.TPUtils;
...
public class AutoFrameDupe extends Module {
    ...
    public final Setting<List<Item>> dupeItems = sgGeneral.add(new ItemListSetting.Builder()
            ...
            .defaultValue(Arrays.asList(Items.SHULKER_BOX, Items.WHITE_SHULKER_BOX, Items.ORANGE_SHULKER_BOX,
                    Items.MAGENTA_SHULKER_BOX,
                    Items.LIGHT_BLUE_SHULKER_BOX, Items.YELLOW_SHULKER_BOX, Items.LIME_SHULKER_BOX,
                    Items.PINK_SHULKER_BOX,
                    Items.GRAY_SHULKER_BOX, Items.LIGHT_GRAY_SHULKER_BOX, Items.CYAN_SHULKER_BOX,
    ...
    private final Setting<Integer> maxInventoryMoves = sgGeneral.add(new IntSetting.Builder()
            ...
            .description(
                    "The maximum number of item moves from inventory to hotbar to perform per tick (recommended to leave at 0).")
            .defaultValue(0)
            .min(0)
            .sliderMax(20)
            .build());
    ...
    public void onDeactivate() {
        ...
        MeteorExecutor.execute(() -> {
            ...
            if (mode.get() == Mode.Fast && replaceItemFrames.get()) {
                ...
                replaceLoop: for (ItemFrameEntity itemFrame : toReplace) {

                    for (ItemFrameEntity existingItemFrame : itemFrames) {
                        if (existingItemFrame.getPos().equals(itemFrame.getPos())
                                && existingItemFrame.getHorizontalFacing() == itemFrame.getHorizontalFacing()) {
                            continue replaceLoop;
                        }
                    }

                    BlockPos pos = TPUtils
                            .Vec3d2BlockPos(itemFrame.getPos().subtract(itemFrame.getRotationVector().normalize()));
                    if (inv.getStack(PlayerInventory.OFF_HAND_SLOT).getItem() instanceof ItemFrameItem) {
                        mc.interactionManager.interactBlock(mc.player, Hand.OFF_HAND,
                                new BlockHitResult(Vec3d.ofCenter(pos), itemFrame.getHorizontalFacing(), pos, false));
                        continue;
                    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (chronometer.get()) {
            mc.inGameHud.setOverlayMessage(Text.of("Duration: §f"
                    + DurationFormatUtils.formatDurationWords(System.currentTimeMillis() - startTime, true, true)),
                    false);
        }
        ...
        if (mode.get() == Mode.Normal) {
            ...
            for (ItemFrameEntity emptyItemFrame : emptyItemFrames) {
                ...
                if (!dupeItems.get().contains(inv.getMainHandStack().getItem()) && moves < maxInventoryMoves.get()
                        && !swapped) {
                    for (int i = 0; i < inv.size(); i++) {
                        if (dupeItems.get().contains(inv.getStack(i).getItem())) {
                            InvUtils.move().from(i).toHotbar(inv.selectedSlot);
            ...
            if (replaceItemFrames.get()) {
                ...
                replaceLoop: for (ItemFrameEntity itemFrame : List.copyOf(toReplace)) {
            ...
            for (ItemFrameEntity itemFrame : itemFrames) {
                if ((!dupeItems.get().contains(inv.getMainHandStack().getItem())
                        || inv.getMainHandStack().getCount() < minStackSize.get()) && swaps < maxSwaps.get()) {
                    for (int i = 0; i < 9; i++) {
                        if (dupeItems.get().contains(inv.getStack(i).getItem())
                                && inv.getStack(i).getCount() >= minStackSize.get()) {
                            InvUtils.swap(i, false);
                            swapped = true;
                            swaps++;
                            break;
                ...
                if ((!dupeItems.get().contains(inv.getMainHandStack().getItem())
                        || inv.getMainHandStack().getCount() < minStackSize.get()) && moves < maxInventoryMoves.get()
                        && !swapped) {
                    for (int i = 0; i < inv.size(); i++) {
                        if (dupeItems.get().contains(inv.getStack(i).getItem())
                                && inv.getStack(i).getCount() >= minStackSize.get()) {
                ...
                if (dupeItems.get().contains(inv.getMainHandStack().getItem())
                        && itemFrame.getHeldItemStack().getItem() == Items.AIR && placements < maxPlacements.get()) {
                ...
                if (!dontHit.contains(itemFrame) && dupeItems.get().contains(itemFrame.getHeldItemStack().getItem())
                        && placements < maxPlacements.get()) {
                    dontHit.add(itemFrame);
                    mc.interactionManager.attackEntity(mc.player, itemFrame);
                    if (dupeItems.get().contains(inv.getMainHandStack().getItem())) {
                        interactItemFrame(itemFrame);
