...
import com.allinone.util;
import java.util.List;
import java.util.Random;

import static com.allinone.util.banstuff;
...
public class AutoChatGame extends Module {
    ...
    private final Setting<Boolean> experimental = sgGeneral.add(new BoolSetting.Builder()
            ...
            .defaultValue(false)
            .build());
    private final Setting<List<String>> messages = sgGeneral.add(new StringListSetting.Builder().name("messages")
            .description("Trigger messages").defaultValue("say", "type", "write").build());
    private final Setting<List<String>> reversers = sgGeneral.add(new StringListSetting.Builder().name("reversers")
            .description("Strings that will reverse the solution").defaultValue("reverse").build());
    private final Setting<List<String>> mether = sgGeneral.add(new StringListSetting.Builder().name("mather")
            .description("Strings that will solve an equation as solution")
            .defaultValue("solve", "equation", "calculate").build());
    private final Setting<List<String>> variablegame = sgGeneral.add(new StringListSetting.Builder()
            ...
            .build());
    private final Setting<List<String>> quotes = sgGeneral.add(
            new StringListSetting.Builder().name("quotes").description("Quotes").defaultValue("\"", "'", "`").build());
    private final Setting<List<String>> contain = sgGeneral.add(new StringListSetting.Builder().name("must contain")
            .description("Requires the message to contain all of these Strings").defaultValue().build());
    private final Setting<List<String>> dont = sgGeneral.add(new StringListSetting.Builder()
            .name("blacklisted messages").description("Do not solve the chatgame if it contains one of these Strings")
            .defaultValue("solved", "successfully").build());
    ...
    public AutoChatGame() {
        super(Addon.CATEGORY, "auto-chatgame", "Automatically answers most chat games when triggered");
    }
    ...
    private void PacketReceive(PacketEvent.Receive event) {
        try {
            ...
            for (int i = 0; i < quotes.get().size(); i++) {
                if (content.toLowerCase().contains(quotes.get().get(i)) && content.toLowerCase()
                        .indexOf(quotes.get().get(i), content.indexOf(quotes.get().get(i)) + 1) != -1) {
                    no = true;
                ...
                if (hidetr.get()) {
                    ...
                    if (shmsg.get()) {
                        ...
                        ChatUtils.sendMsg(Text.of(
                                "§7[§9Chatgame§7]§f " + mode + ": " + quote + content.split(quote)[1] + quote + "!"));
                    }
                }
    ...
    int DoVariableGame(String output) { // No, this is actually not AI generated. // guys I came back to the code and
                                        // shit looks ai generated asf man wtf
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (!isActive()) {
            return;
        } // this got me banned off my favorite server
        if (this.tick > 0) {
            this.tick--;
            return;
        } // don't execute when it's not done waiting
        if (this.tick == -1) {
            return;
        } // disable when on -1
