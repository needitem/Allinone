...
/**
 * This module was originally made to bypass
 * <a href="https://github.com/KRYMZ0N/AnarchyAnticheat">this anticheat</a>
 */
public class VecFly extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        boolean sprinting = mc.options.sprintKey.isPressed() && !mc.options.jumpKey.isPressed()
                && speedBypassMode.get() != SpeedBypassMode.None;
        if (sprinting != bypassLastTick && !mc.player.isOnGround()
                && !mc.world.getBlockState(stepDownPos).isSolidBlock(mc.world, stepDownPos)) {
        ...
        if (speedBypassMode.get() == SpeedBypassMode.None)
            return;
        if (speedBypassMode.get() == SpeedBypassMode.OnSprint && !mc.options.sprintKey.isPressed())
            return;
        if (forward.equals(Vec3d.ZERO) && tempLastForward.equals(Vec3d.ZERO))
            return;
        if (mc.options.jumpKey.isPressed())
            return;
