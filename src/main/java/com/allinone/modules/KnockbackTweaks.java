...
public class KnockbackTweaks extends Module {
    ...
    public KnockbackTweaks() {
        super(Addon.CATEGORY, "knockback-tweaks",
                "Allows you to change the direction of the knockback received by the entities you hit. (doesn't work with criticals)");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (shouldResendLookPacket) {
            ...
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(mc.player.getYaw(),
    ...
    private void onEntityAttack(AttackEntityEvent event) {
        ...
        mc.player.networkHandler
                .sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(yaw, mc.player.getPitch(), mc.player.isOnGround()));
        shouldResendLookPacket = true;
    }

    @EventHandler
    private void onRender(Render3DEvent event) {
        if (mc.player == null || mc.world == null || mc.cameraEntity == null || !renderDirection.get())
            return;

        float yawMin = (mode.get() == Mode.Absolute ? mc.player.getYaw() - direction.get() : -direction.get())
                - randomRange.get();
        ...
        Vec3d kbRangeMin = lookPos.add(
                rotation.normalize().multiply(radius.get(), 0, radius.get()).rotateY((float) Math.toRadians(yawMin)));
        Vec3d kbRangeMax = lookPos.add(
                rotation.normalize().multiply(radius.get(), 0, radius.get()).rotateY((float) Math.toRadians(yawMax)));
        ...
        if (randomRange.get() != 0) {
            ...
            for (float yaw = yawMin; yaw < yawMax; yaw++) {
                Vec3d currentPoint = lookPos.add(rotation.normalize().multiply(radius.get(), 0, radius.get())
                        .rotateY((float) Math.toRadians(yaw)));
                if (lastPoint != null)
                    event.renderer.line(lastPoint.x, lastPoint.y, lastPoint.z, currentPoint.x, currentPoint.y,
                            currentPoint.z, lineColor.get());
                lastPoint = currentPoint;
            }
            event.renderer.line(lastPoint.x, lastPoint.y, lastPoint.z, kbRangeMax.x, kbRangeMax.y, kbRangeMax.z,
                    lineColor.get());
        }
