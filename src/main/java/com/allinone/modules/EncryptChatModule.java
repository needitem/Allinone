...
public class EncryptChatModule extends Module {
    public EncryptChatModule() {
        super(Addon.CATEGORY, "chat-encryption",
                "Encrypts your chat messages so only asteroide users can read them");
    }
            ...
            mc.getNetworkHandler()
                    .onGameMessage(
                            new GameMessageS2CPacket(
                                    Text.literal(String.format("§e§l%s§r§e%s §7(Decrypted)",
                                            message.substring(0, start - 5), util.decrypt(split[1], split[2]))),
                                    overlay));
        }
    }
        ...
        if (start >= 0 && (content.indexOf("\"", start)) >= 0) {
            mc.getNetworkHandler()
                    .onGameMessage(
                            new GameMessageS2CPacket(
                                    Text.literal(String.format("§e§l%s§r§e%s §7(Decrypted)",
                                            content.substring(0, start - 5), util.decrypt(split[1], split[2]))),
                                    overlay));
        }
    }
}
