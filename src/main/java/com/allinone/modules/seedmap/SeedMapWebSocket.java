...
package com.allinone.modules.seedmap;
...
public class SeedMapWebSocket extends WebSocketServer {
    ...
    public void onOpen(WebSocket conn, ClientHandshake handshake) {
        ...
        if (debugMessages) {
            ChatUtils.infoPrefix("SeedMap",
                    "New connection from " + getAddr(conn) + ":" + conn.getRemoteSocketAddress().getPort());
        }
    }
    ...
    public void onClose(WebSocket conn, int code, String reason, boolean remote) {
        ...
        if (debugMessages) {
            ChatUtils.infoPrefix("SeedMap",
                    "Closed connection to " + getAddr(conn) + ":" + conn.getRemoteSocketAddress().getPort());
        }
    }
    ...
    public void onMessage(WebSocket conn, String message) {
        if (debugMessages) {
            ChatUtils.infoPrefix("SeedMap",
                    getAddr(conn) + ":" + conn.getRemoteSocketAddress().getPort() + " : " + message);
        }
    }
