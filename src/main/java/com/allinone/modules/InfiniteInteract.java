...
import static com.allinone.utils.TPUtils.findTPPath;
...
public class InfiniteInteract extends Module {
    ...
    public InfiniteInteract() {
        super(Addon.CATEGORY, "infinite-interact",
                "Allows you to interact with blocks and entities at a distance. (require reach module)");
    }
    ...
    private void onAttack(AttackEntityEvent event) {
        if (mc.player == null || !hitEntities.get())
            return;
        if (notIfSneaking.get() && mc.player.isSneaking())
            return;
    ...
    private void onInteractBlock(InteractBlockEvent event) {
        if (mc.player == null || !interactBlocks.get())
            return;
        if (notIfSneaking.get() && mc.player.isSneaking())
            return;
        ...
        if (blockPos.distanceTo(mc.player.getPos()) <= 5)
            return;
    ...
    private void onInteractEntity(InteractEntityEvent event) {
        if (mc.player == null || !interactEntities.get())
            return;
        if (notIfSneaking.get() && mc.player.isSneaking())
            return;
    ...
    private void onStartBreakingBlock(StartBreakingBlockEvent event) {
        if (mc.player == null || !breakBlocks.get())
            return;
        if (notIfSneaking.get() && mc.player.isSneaking())
            return;
        ...
        mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK,
                event.blockPos, event.direction));
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (lastTP + trailRenderTime.get() * 1000 < System.currentTimeMillis())
            return;
        ...
        if (showSteps.get()) {
            for (Vec3d vec : positions) {
                event.renderer.box(vec.x - 0.375, vec.y, vec.z - 0.375, vec.x + 0.375, vec.y + 2, vec.z + 0.375,
                        boxColor.get(), boxColor.get(), ShapeMode.Sides, 0);
            }
        }
    }
